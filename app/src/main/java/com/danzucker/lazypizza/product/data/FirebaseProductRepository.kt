...
class FirebaseProductRepository(
     ...
     */
    override fun getProducts(): Flow<Result<List<Product>, DataError.Network>> =
        callbackFlow {
            val listener =
                firestore
     ...
     */
    override fun getToppings(): Flow<Result<List<Topping>, DataError.Network>> =
        callbackFlow {
            val listener =
                firestore
                    .collection(TOPPINGS_COLLECTION)
                    .addSnapshotListener { snapshot, error ->
                        if (error != null) {
                            trySend(Result.Error(DataError.Network.UNKNOWN))
                            return@addSnapshotListener
     ...
     */
    override fun getProductsByCategory(category: ProductCategory): Flow<Result<List<Product>, DataError.Network>> =
        callbackFlow {
            val listener =
                firestore
                    .collection(PRODUCTS_COLLECTION)
